...
def staged_file(config: dict[str, str], raw: str) -> Path:
    ...
    if value.parent.resolve(strict=True) != root:
        raise HelperError("Staged DHCP configuration is outside STAGING_DIR", 2)
    if value.stat().st_size > 8 * 1024 * 1024:
        raise HelperError("Staged DHCP configuration exceeds 8 MiB", 2)
    return value.resolve(strict=True)


@contextlib.contextmanager
def trusted_kea_candidate(config: dict[str, str], family: int, source: Path) -> Iterator[Path]:
    """Validate via a temporary root-owned file in Kea's AppArmor-readable directory."""
    target = config_path(config, family)
    directory = target.parent.resolve(strict=True)
    fd, name = tempfile.mkstemp(prefix=f"kea-dhcp{family}.conf.chrislab-", suffix=".json", dir=directory)
    candidate = Path(name)
    try:
...
def backup_current(config: dict[str, str], family: int, target: Path) -> Path | None:
    ...
    destination = root / f"kea-dhcp{family}-{stamp}.json"
    counter = 0
    while destination.exists():
        counter += 1
        destination = root / f"kea-dhcp{family}-{stamp[:-1]}-{counter:02d}Z.json"
...
def cmd_service(args: argparse.Namespace, config: dict[str, str]) -> None:
    ...
    name = service(family)
    target = config_path(config, family)
    if action in {"start", "restart", "enable-start"}:
        load_json_file(target, family)
        validate_kea(family, target)
    if action == "start":
...
def active_lease_path(config: dict[str, str], family: int) -> Path | None:
    ...
    if not isinstance(lease, dict) or lease.get("type", "memfile") != "memfile":
        return None
    candidate = Path(str(lease.get("name") or f"/var/lib/kea/kea-leases{family}.csv")).resolve(strict=False)
...
def parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--family", required=True)
    p.add_argument("--limit", default="250")
    p.set_defaults(func=cmd_leases)

    p = sub.add_parser("logs")
    p.add_argument("--family", required=True)
    p.add_argument("--lines", default="100")
